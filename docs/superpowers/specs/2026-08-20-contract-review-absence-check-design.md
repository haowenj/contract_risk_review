...
2. text、table、image 使用现有可检索文本进行扫描。
3. NFKC、大小写和简单空白规范化。
4. 确定性子串匹配，不发生模糊误命中。
5. 真实数组下标作为 `source_object_index`，候选去重、排序和最多 20 条限制。
6. 两次 RAG 上限及 State 路由。
7. 全文候选存在时交给风险判断 LLM，引用字段不由 LLM 生成。
8. 全文无候选时使用 `absence_verified`，措辞受限且风险等级来自 LLM。
9. Rerank Debug 和 absence Debug 不混入最终 Evidence。
10. 合同预检不触发 RAG 或全文扫描。
11. 聊天、Evaluation、text/table/image RAG 及入库流程回归不变。
