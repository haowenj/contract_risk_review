...
第一期以《中国寰球工程有限公司投标或拟签约民营及海外工程项目合同风险评估负面清单》为真实业务样本。该 PDF 共四页，包含“投标立项审查负面事项”和“投标报价审查及签约审批负面事项”两个阶段，按最细层级拆分后有 42 个检查项。实现必须支持通用风险规范，不能把这 42 项硬编码进业务逻辑。
...
只有最细层级的可执行标准成为默认检查项。仅用于归纳子项的父标题不重复执行。像样本“二-1”这种重新评估前序事项的流程规则仍保留为检查项，并标记 `item_kind=process_control`。
...
对真实样本的本地验收必须显示两个阶段和 42 个最细检查项，并保留 `一-5.3`、`二-6.6` 等原始编号。
...
真实商务 PDF 不提交到 Git。仓库使用脱敏合成 PDF 覆盖相同的两级编号结构；真实文件只作为本地验收样本，验证四页、两个阶段和 42 个最细检查项。
...
1. 可以上传并激活 PDF/TXT/MD 风险规则集。
2. 真实样本在本地稳定解析为两个阶段、42 个最细检查项，并保留原编号和页码。
